...
class HTML5VideoAdapter {
    constructor(videoElement) {
        this.playerType = PlayerType.HTML5;
    ...
    attachEventListeners() {
        ...
        if (this.adapter.element instanceof HTMLVideoElement) {
            const fullscreenChangeHandler = () => {
                this.emitEvent(VideoEventType.FULLSCREEN_CHANGE, this.getBaseEventData());
            };
...
class VideoJSAdapter {
    constructor(videojsPlayer) {
        this.playerType = PlayerType.VIDEO_JS;
...
class PlyrAdapter {
    constructor(plyrPlayer) {
        this.playerType = PlayerType.PLYR;
...
class JWPlayerAdapter {
    constructor(jwPlayer) {
        this.playerType = PlayerType.JW_PLAYER;
...
class YouTubeAdapter {
    ...
    isPaused() {
        ...
        return (state === YouTubePlayerState.PAUSED ||
...
class EventAnalytics {
    ...
    getAnalytics() {
        const playEvents = this.events.filter((e) => e.type === VideoEventType.PLAY);
        const pauseEvents = this.events.filter((e) => e.type === VideoEventType.PAUSE);
        const seekEvents = this.events.filter((e) => e.type === VideoEventType.SEEK);
        const bufferStartEvents = this.events.filter((e) => e.type === VideoEventType.BUFFER_START);
        const qualityChangeEvents = this.events.filter((e) => e.type === VideoEventType.QUALITY_CHANGE);
        const errorEvents = this.events.filter((e) => e.type === VideoEventType.ERROR);
...
export { EventAnalytics, HTML5VideoAdapter, JWPlayerAdapter, PlayerType, PlyrAdapter, VideoEventType, VideoJSAdapter, VideoTracker, VimeoAdapter, YouTubeAdapter, YouTubePlayerState, createHTML5Tracker };
